...
def wgmma(acc, a, b, *, rhs_transpose: bool | None = None, swizzle: int = 128):
  ...
  if acc.memory_space != gpu_core.REGS:
  ...
  if tma * ma != mc or nb * tnb != nc or ka != kb or tka != tkb:
    raise ValueError(a.shape, b.shape, acc.shape, rhs_transpose)
...
def _wgmma_wait_lowering_rule(ctx: lowering.LoweringRuleContext, wait_until):
  del ctx

  ...
  def test_wgmma(self):
    ...
    def kernel(a_ref, b_ref, o_ref):
      def body(acc):
        ...
        plgpu_primitives.wgmma(acc, a_ref, b_ref, rhs_transpose=rhs_transpose)
    ...
    _rng = lambda shape, dtype: jnp.linspace(0, 1, int(np.prod(shape)), dtype=dtype).reshape(shape)
    a = _rng((64, 128), dtype)
    b = _rng((128, 128), dtype)

    res = pl.pallas_call(
        kernel,
        in_specs=[
            plgpu.GPUBlockSpec(
                (64, 128), lambda i, j: (i, j), tiling=(64, elems_128b), swizzle=128
            ),
            plgpu.GPUBlockSpec(
                (128, 128), lambda *i: i, tiling=(elems_128b, elems_128b), swizzle=128, transpose=rhs_transpose
            ),
        ],
    ...
    np.testing.assert_allclose(res, a @ (b.T if rhs_transpose else b), rtol=1e-3)

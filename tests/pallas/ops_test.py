...
class OpsTest(PallasBaseTest):
  ...
  def test_sign(self, dtype, value):
    ...
    if dtype == jnp.bfloat16 and value == jnp.nan:
      # TODO(ayx): workaround for https://github.com/jax-ml/ml_dtypes/issues/206
      assert jnp.isnan(out).all(), "The value must be equal to nan"
    else:
